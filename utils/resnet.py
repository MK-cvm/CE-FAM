...
class ResNet(nn.Module):
    ...
    def mask_predict(self, x, mask_index=None, layer_index=None):
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
        x = self.maxpool(x)
        x = self.layer1(x)
        if layer_index == 0:
            avg_value = torch.mean(x[:, mask_index], dim=(2,3), keepdim=True)
            x[:, mask_index] = avg_value
        x = self.layer2(x)
        if layer_index == 1:
            avg_value = torch.mean(x[:, mask_index], dim=(2,3), keepdim=True)
            x[:, mask_index] = avg_value
